...
export async function generateMetadata({
  ...
  const { t } = await getTranslations(locale, "metadata.dossier");
  return {
    title: t("titleTemplate", { id }),
...
export default async function DossierPage({ params }: DossierPageProps) {
  ...
  const { t } = await getTranslations(locale, "app.dossierPage");
  const { t: tPalette } = await getTranslations(locale, "palette");
  const { t: tMetals } = await getTranslations(locale, "metals");
  const { t: tValues } = await getTranslations(locale, "ai.values");
  ...
  const paletteName = tPalette(`${season}.displayName`);
  const paletteDesc = tPalette(`${season}.description`);
  ...
  // Helper per tradurre enum AI con fallback al valore raw (per backward compat
  // con record DB pre-Step5 dove l'enum è in IT — il transform Zod normalizza
  // verso EN keys per i NUOVI record, ma il classification_result salvato a DB
  // potrebbe ancora contenere "caldo" etc. La nostra mappatura ai.values.* è
  // per chiavi EN, ma se il valore non matcha lo mostriamo capitalizzato.
  const enumLabel = (group: string, key?: string): string | undefined => {
    if (!key) return undefined;
    const translated = tValues(`${group}.${key}`);
    // Se la chiave non esiste, t() ritorna la chiave letterale → fallback
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <nav className="mb-8 flex items-center gap-2 text-sm animate-fade-in">
          ...
          <span className="text-ink font-medium">
            {t("breadcrumbCurrent", { id: dossierId })}
          </span>
        </nav>
        ...
        {analysis && (
          <div className="mb-12 rounded-2xl border border-accent/8 bg-white p-8 shadow-xs animate-slide-up" style={{ animationDelay: "0.15s" }}>
            ...
            <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-5">
              {[
                { label: t("attrSkinTone"), value: analysis.skinTone, icon: "🎨" },
                { label: t("attrHairColor"), value: analysis.hairColor, icon: "💇" },
                { label: t("attrEyeColor"), value: analysis.eyeColor, icon: "👁️" },
                { label: t("attrUndertone"), value: enumLabel("undertone", analysis.undertone), icon: "🌡️" },
                { label: t("attrContrast"), value: enumLabel("contrast", analysis.contrast), icon: "◐" },
                ...
                <div key={d.label} className="rounded-xl bg-cream/60 p-4 text-center">
                  ...
                  <p className="mt-1 text-sm text-ink font-medium">{d.value}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {reasoning && (
          <div className="mb-12 rounded-2xl border border-accent/8 bg-white p-8 shadow-xs animate-slide-up" style={{ animationDelay: "0.2s" }}>
            <h2 className="font-serif text-xl text-ink mb-6">{t("reasoningTitle")}</h2>
        ...
        <div className="mt-12 rounded-2xl border border-accent/8 bg-white p-8 shadow-xs">
          ...
          <div className="flex flex-wrap gap-3">
            {palette.metals.map((metal) => (
              <span
                key={metal}
                className="inline-flex items-center gap-2 rounded-xl border border-accent/10 bg-cream/70 px-4 py-2.5 text-sm font-medium text-ink transition-all hover:shadow-xs hover:border-accent/20"
              >
                <span
        ...
        {reasoning?.practicalTips && reasoning.practicalTips.length > 0 && (
          <div className="mt-12 rounded-2xl border border-accent/8 bg-white p-8 shadow-xs">
            <h2 className="font-serif text-xl text-ink mb-6">{t("tipsTitle")}</h2>
        ...
        {reasoning?.lookSuggestions && reasoning.lookSuggestions.length > 0 && (
          <div className="mt-12 rounded-2xl border border-accent/8 bg-white p-8 shadow-xs">
            <h2 className="font-serif text-xl text-ink mb-6">{t("looksTitle")}</h2>
        ...
        <p className="mt-10 text-center text-xs text-muted-light/60">
          {t("footerGenerated", { date: dateFmt.format(new Date(dossier.created_at)) })}
        </p>
      </div>
    </div>
  );
